...
def _enrich_discover_item(data, record=True):
    ...
    if cached:
        if not _same_discover_release(cached, data):
            ...
            cached = None
        elif _has_real_topic_url(cached):
            topic = _make_discover_topic(cached)
            _fill_video_fields_from_topic_page(topic)
            refreshed = _topic_card_payload(topic, 'Обогащён', cached.get('source') or 'Discover')
            cached = _merge_identity_payload(refreshed, cached)
            cached['status'] = 'Обогащён'
    if cached:
        result = {
...
function itemKeys(m){return [m.topic_id,m.source_topic_id,m.original_topic_id,m.magnet,m.imdb_id,m.kp_id,m.topic_url].filter(Boolean).map(String)}
function sameMovie(a,b){const ak=itemKeys(a),bk=itemKeys(b);return ak.some(x=>bk.includes(x))}
