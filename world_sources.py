...
def parse_piratebay_page(
    ...
    for row in rows:
        ...
        topics.append(_base_world_topic(
            topic_id, title, movie_title, movie_year, collection, "piratebay",
            category, author, size_clean, size_bytes, seeders, leechers,
            parse_world_date(uploaded), now_text, topic_url, len(topics), magnet,
            detect_format_from_text,
        ))
    return topics
...
def parse_tpbparty_page(
    ...
    for row in rows:
        ...
        size_bytes, size_clean = parse_size(size_str)
        topics.append(_base_world_topic(
            topic_id, title, movie_title, movie_year, collection, "tpbparty",
            category, author, size_clean, size_bytes, seeders, leechers,
            parse_world_date(uploaded), now_text, topic_url, len(topics), magnet,
            detect_format_from_text,
        ))
...
def _merge_duplicates_in_group(group):
    ...
    def _seeders(t):
        try:
            return int(t.get("seeders")) if t.get("seeders") is not None else -1
...
def _dedup_one_collection_by_id(topics_list, collection_name):
    ...
    for _imdb, indices in by_imdb.items():
        if len(indices) >= 2:
            groups.append([topics_list[i] for i in indices])
            used.update(indices)
    ...
    for _kp, indices in by_kp.items():
        if len(indices) >= 2:
            groups.append([topics_list[i] for i in indices])
            used.update(indices)
    result = [_merge_duplicates_in_group(g) for g in groups]
