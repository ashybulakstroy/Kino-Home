...
REFRESH_COLLECTIONS = {
    key: value for key, value in COLLECTIONS.items() if not value.get('activity')
}
...
def normalize_legacy_world_topic(topic, collection, source):
    ...
    normalized = {
        ...
        'date_str': topic.get('date_str') or parse_piratebay_date(topic.get('uploaded') or ''),
...
def fix_bad_topics(topics):
    ...
    need_re_enrich = []
    for t in topics:
        if is_world_topic(t) or t.get('collection') in ACTIVITY_COLLECTIONS:
            continue
...
def repair_world_titles(topics):
    tech = re.compile(
        ...
        r'YIFY|YTS|RARBG|RMTeam|NeoNoir|SupaCvnt|FLUX|BrRip|UNiON|FS|FaS|Line)',
        re.I,
    )

    def clean_world_title(raw_title):
        text = re.sub(r'[._]+', ' ', raw_title or '')
        text = re.sub(r'\[[^\]]*\]|\([^\)]*\)', ' ', text)
        ...
        text = tech.sub(' ', text)
        text = re.sub(r'\b(?:5\s*1|2\s*0|7\s*1|10bits?|8bits?)\b', ' ', text, flags=re.I)
        text = re.sub(r'[^A-Za-z0-9А-Яа-яЁё]+', ' ', text)
        text = re.sub(r'\s+', ' ', text).strip()
        if not text:
            text, fallback_year = clean_title(raw_title)
...
def clean_catalog_topics(topics):
    ...
    playable = merge_world_duplicates(playable)
...
<span class="gl">Формат:</span><select class="gs" onchange="af()" id="fs"><option value="">Все</option>''' + fmt_opts + '''</select>
...
(function(){{var dv=localStorage.getItem('dv'),cv=localStorage.getItem('cv'),sv=localStorage.getItem('sv'),fv=localStorage.getItem('fv'),cs=document.getElementById('cs'),fs=document.getElementById('fs');if(dv)document.getElementById('ds').value=dv;if(cv&&cs){{for(var oi=0;oi<cs.options.length;oi++){{if(cs.options[oi].value===cv){{cs.value=cv;break}}}}}}if(fv&&fs){{for(var fi=0;fi<fs.options.length;fi++){{if(fs.options[fi].value===fv){{fs.value=fv;break}}}}}}if(sv)document.getElementById('ss').value=sv;af();sortTiles();
...
var playerPoll=null;
var playerHeartbeat=null;
var currentHash='';
var currentSession='';
var currentWatchStartedAt=0;
var currentWatchMeta=null;
...
function stopPlayerHeartbeat(){{if(playerHeartbeat){{clearInterval(playerHeartbeat);playerHeartbeat=null}}}}
function startPlayerHeartbeat(h){{stopPlayerHeartbeat();playerHeartbeat=setInterval(function(){{if(!currentSession||currentHash!==h)return;fetch('/status/'+h+'?sid='+encodeURIComponent(currentSession),{{cache:'no-store'}}).catch(function(){{}})}},15000)}}
function stopCurrentSession(){{stopPlayerHeartbeat();if(!currentSession&&!currentHash)return;var sid=currentSession,h=currentHash;currentSession='';currentHash='';var payload=JSON.stringify({{sid:sid,hash:h}});try{{if(navigator.sendBeacon){{var blob=new Blob([payload],{{type:'application/json'}});navigator.sendBeacon('/stop_session',blob);return}}}}catch(_e){{}}fetch('/stop_session',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:payload,keepalive:true}}).catch(function(){{}})}}
...
def load_collection_listing(collection, coll_info, topics_limit):
    ...
    if is_world_source(source):
        ...
        for attempt in range(1, MAX_RETRY + 1):
            ...
            listing_errors += 1
            _cache_used, raw = load_world_listing_cache_bytes(collection, source)
            if raw:
                html = raw.decode('utf-8', errors='replace')
                page_topics = parse_world_page(
                    html, collection, source, world_identity_clean_title, parse_size, now_text,
...
def main():
    ...
    elif collection_arg and not COLLECTIONS[collection_arg].get('activity'):
        collections_to_process = [collection_arg]
    else:
        collections_to_process = list(REFRESH_COLLECTIONS.keys())
