...
`skip_topics` применяется только к HTML-листингу с его служебными верхними строками. Atom-лента всегда читается целиком с первой записи. Refresh через Atom считается `degraded`, а итоговый лог должен указывать `source=atom` и количество pending-тем.
