...
| `python generate_page.py --repair-formats --collection=tpbparty_top` | Параллельно восстановить пустые форматы World-коллекции из file list/metadata |
...
Rutracker вставляет `<wbr>` внутрь длинных заголовков. Текст названия нужно читать с сохранением исходных пробелов (`inline_html_text()`), а не через `get_text(strip=True)`: иначе `Родительский<wbr> дом` превращается в `Родительскийдом`, что ломает поиск ID, постеров и трейлеров. При этом нельзя безусловно добавлять пробел после `<wbr>`, поскольку `AV<wbr>C` должно остаться `AVC`.
...
На detail-страницах World-источников сначала нужно разбирать `#filelistContainer`: формат берётся из расширения самого большого видеофайла, размер — из соседней ячейки. Libtorrent metadata используется только если список файлов на странице отсутствует. `parse_size()` должен поддерживать как `GB`, так и двоичные единицы `GiB/MiB/KiB`.
...
Обязательные данные темы: magnet, формат видео, постер и трейлер. Пока хотя бы одно из этих полей отсутствует, недельный `ENRICH_NO_CHANGE_RETRY_DAYS` не применяется и лимит попыток не останавливает тему навсегда. Повтор выполняется по существующему `ENRICH_RETRY_COOLDOWN_MINUTES`. Получение torrent-метаданных для определения формата допускается параллельно всеми `WORKER_COUNT` воркерами, каждый со своей libtorrent-сессией.
